...
router.post("/register",
  ...
  async (req, res, next) => {
    try {
      ...
      await db.query(
        "INSERT INTO users (name, email, password, role, role_id, phone, first_name, last_name, active, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 1)",
        [`${firstName} ${lastName}`, email, passwordHash, 'usager', 3, phone, firstName, lastName]
      );
